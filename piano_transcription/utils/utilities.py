...
def read_midi_bach10(midi_path):
    """Parse MIDI file and select only events for instrument 'Instrument 1'.
    ...
    # Find the track with instrument name "Instrument 1"
    instrument_track_idx = None
    for idx, track in enumerate(midi_file.tracks):
        for msg in track:
            if msg.type == 'track_name' and getattr(msg, 'name', '') == 'Instrument 1':
                instrument_track_idx = idx
                break
        if instrument_track_idx is not None:
            break

    if instrument_track_idx is None:
        raise ValueError("No track with instrument name 'Instrument 1' found in MIDI file.")

    # Get tempo from the first track (assume first event is set_tempo)
    microseconds_per_beat = midi_file.tracks[0][2].tempo
    beats_per_second = 1e6 / microseconds_per_beat
    ticks_per_second = ticks_per_beat * beats_per_second
    ...
    for message in midi_file.tracks[instrument_track_idx]:
        message_list.append(str(message))
        ticks += message.time
        time_in_second.append(ticks / ticks_per_second)
...
def write_events_to_midi_with_technique(start_time, note_events, pedal_events, midi_path):
    '''
    technique_names: 
            0: "legato",
            1: "pizzicato", 
            ...
            3: "sustain",
            -1: "unknown",
    Present each technique as a separate track in the MIDI file.
    '''
    ...
    # Group note events by technique
    technique_to_notes = {}
    print(f"keys of note_events: {note_events[0].keys()}")
    for note_event in note_events:
        technique = note_event.get('technique', -1)
        if technique not in technique_to_notes:
            technique_to_notes[technique] = []
        technique_to_notes[technique].append(note_event)
    ...
    # Track for pedal events (always track 1)
    track_pedal = MidiTrack()
    pedal_message_roll = []
    if pedal_events:
        for pedal_event in pedal_events:
            pedal_message_roll.append({'time': pedal_event['onset_time'], 'control_change': 64, 'value': 127})
            pedal_message_roll.append({'time': pedal_event['offset_time'], 'control_change': 64, 'value': 0})
    pedal_message_roll.sort(key=lambda x: x['time'])
    previous_ticks = 0
    for message in pedal_message_roll:
        this_ticks = int((message['time'] - start_time) * ticks_per_second)
        if this_ticks >= 0:
            diff_ticks = this_ticks - previous_ticks
            previous_ticks = this_ticks
            if 'control_change' in message:
                track_pedal.append(Message('control_change', channel=0, control=message['control_change'], value=message['value'], time=diff_ticks))
    track_pedal.append(MetaMessage('end_of_track', time=1))
    midi_file.tracks.append(track_pedal)

    # Technique tracks: one per technique
    for technique, notes in technique_to_notes.items():
        track = MidiTrack()
        # Add a track name for clarity
        technique_name = technique_names.get(technique, f"tech_{technique}")
        track.append(MetaMessage('track_name', name=technique_name, time=0))
        ...
        # Sort by time
        message_roll.sort(key=lambda x: x['time'])
        previous_ticks = 0
...
class RegressionPostProcessor(object):
    ...
    def output_dict_to_note_pedal_arrays(self, output_dict):
        ...
        Returns:
          ...
          est_pedal_on_offs: (pedal_events_num, 2), the 2 columns are onset_time 
            and offset_time. E.g. [
             ...
             ...]
        """
        # a = input(f"output_dict in note_pedal_arrays: {output_dict}")
        ...
        est_on_off_note_vels = self.output_dict_to_detected_notes(output_dict) # TODO: this this error

        if 'reg_pedal_onset_output' in output_dict.keys():
            raise Exception("output_dict in note_pedal_arrays!!!")
    ...
    def output_dict_to_detected_notes(self, output_dict):
        ...
        # Handle case where no notes were detected to avoid indexing errors
...
def load_audio(path, sr=22050, mono=True, offset=0.0, duration=None,
    ...
    with audioread.audio_open(os.path.realpath(path), backends=backends) as input_file:
        ...
        for frame in input_file:
            # Librosa >= 0.9 moved buf_to_float to librosa.util
    ...
    if y:
        ...
        if n_channels > 1:
            ...
            # Use top-level helper which now points to librosa.util.to_mono
            y = librosa.to_mono(y)

        if sr is not None:
            print(f"resampling from {sr_native} to {sr}")
            # y = librosa.core.audio.resample(y, sr_native, sr, res_type=res_type)
            # Resample with modern API location
            # Librosa >=0.10 makes orig_sr / target_sr keyword-only
