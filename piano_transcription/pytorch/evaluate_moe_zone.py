"""Evaluate Zone-Specialized MoE technique classification on the viotech test set.
...
Also prints detailed MoE gate usage broken down by expert role:
...
def evaluate(args):
    ...
    with torch.no_grad():
        for batch_idx, batch_data_dict in enumerate(test_loader):
            ...
            gate_probs = output_dict['zone_gate_probs']
    ...
    print()
    print('=' * 70)
    print(f'  Zone-Specialized MoE — Total notes evaluated: {total_notes}')
    print('=' * 70)
    print()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Evaluate Zone-Specialized MoE technique on viotech test set')
