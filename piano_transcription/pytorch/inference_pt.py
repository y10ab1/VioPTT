...
from utilities import (create_folder, get_filename, RegressionPostProcessor, 
    OnsetsFramesPostProcessor, write_events_to_midi, write_events_to_midi_with_technique, load_audio)
from models_contrast import Note_pedal, Regress_onset_offset_frame_velocity_CRNN #, Regress_onset_offset_frame_velocity_technique_CRNN
from pytorch_utils import move_data_to_device, forward
import config
from technique_integration import add_technique_support
...
class PianoTranscription(object):
    def __init__(self, model_type, checkpoint_path=None, 
        ...
        post_processor_type='regression', enable_technique=True):
        ...
        # Enable technique head if requested (and supported by the model)
        try:
            self.model = Model(frames_per_second=self.frames_per_second, 
                classes_num=self.classes_num, predict_technique=enable_technique)
        except TypeError:
            # Fallback for models that do not accept predict_technique
            self.model = Model(frames_per_second=self.frames_per_second, 
    ...
    def transcribe(self, audio, midi_path, apply_technique=False):
        ...
        print(f"Transcribing audio: {midi_path}")

        audio = audio[None, :]  # (1, audio_samples) # eg. (1, 2181067)
        ...
        # show detail of the output_dict
...
def inference(args):
    ...
    # if output dir is not exist, create it
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    midi_path = f'{output_dir}/{get_filename(audio_path)}.mid'
    ...
    transcribed_dict = transcriptor.transcribe(audio, midi_path, apply_technique=args.apply_technique) # TODO
    ...
    if plot:
        ...
        # Only plot the first 1000 frames
        max_frames = 1000

        # Truncate outputs to first 1000 frames
        frame_output = output_dict['frame_output'][:max_frames]
        reg_onset_output = output_dict['reg_onset_output'][:max_frames]
        reg_offset_output = output_dict['reg_offset_output'][:max_frames]
        # Truncate technique_output if present
        has_technique = 'technique_output' in output_dict
        if has_technique:
            technique_output = output_dict['technique_output'][:max_frames]

        n_plots = 5 if has_technique else 4
        fig, axs = plt.subplots(n_plots, 1, figsize=(15, 10 if has_technique else 8), sharex=True)

        # Compute mel spectrogram for the first 1000 frames
        # hop_length=160, so get enough samples for 1000 frames
        mel_hop_length = 160
        needed_samples = max_frames * mel_hop_length
        # Fix: audio is already 1D, not 2D, so use audio[:needed_samples]
        mel = librosa.feature.melspectrogram(
            y=audio[:needed_samples], sr=16000, n_fft=2048, hop_length=mel_hop_length,
            n_mels=229, fmin=30, fmax=8000
        )
        axs[0].matshow(np.log(mel), origin='lower', aspect='auto', cmap='jet')
        axs[1].matshow(frame_output.T, origin='lower', aspect='auto', cmap='jet')
        axs[2].matshow(reg_onset_output.T, origin='lower', aspect='auto', cmap='jet')
        axs[3].matshow(reg_offset_output.T, origin='lower', aspect='auto', cmap='jet')
        axs[0].set_xlim(0, max_frames)
        axs[4 if has_technique else 3].set_xlabel('Frames')
        ...
        if has_technique:
            tech = technique_output
            if tech.ndim == 2:
                axs[4].matshow(tech.T, origin='lower', aspect='auto', cmap='jet')
                axs[4].set_title('technique_output')
            else:
                axs[4].text(0.5, 0.5, 'technique_output shape: {}'.format(tech.shape), ha='center')
        # plt.tight_layout(0, .05, 0)
        plt.savefig(f'{output_dir}/{get_filename(audio_path)}.png')
        print(f'Plot to {output_dir}/{get_filename(audio_path)}.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--apply_technique', type=bool, default=True)
